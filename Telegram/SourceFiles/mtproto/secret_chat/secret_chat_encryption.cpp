...
std::optional<LayerHeader> ParseLayerHeader(bytes::const_span serialized) {
	...
	if (from >= end
		|| mtpTypeId(*from++) != decrypted::mtpc_decryptedMessageLayer) {
		return std::nullopt;
	}
	auto randomBytes = MTPbytes();
	auto layer = MTPint();
	auto inSeqNo = MTPint();
	auto outSeqNo = MTPint();
	if (!randomBytes.read(from, end)
		|| !layer.read(from, end)
		|| !inSeqNo.read(from, end)
		|| !outSeqNo.read(from, end)) {
		return std::nullopt;
	}
	return LayerHeader{
		.randomBytes = bytes::make_vector(bytes::make_span(randomBytes.v)),
		.layer = layer.v,
		.inSeqNo = inSeqNo.v,
		.outSeqNo = outSeqNo.v,
	};
...
bool ValidateLayerHeader(const LayerHeader &header, bool amCreator) {
	// Our out_seq_no parity is 1 for the creator (see nextOutSeqNo), so the
	// peer's must be the opposite; in_seq_no echoes our own parity.
	const auto myParity = amCreator ? 1 : 0;
	return (header.randomBytes.size() >= 15)
		&& (header.outSeqNo >= 0)
		&& ((header.outSeqNo & 1) != myParity)
		&& ((header.inSeqNo < 0) || ((header.inSeqNo & 1) == myParity));
}
...
bytes::vector ComputeKeyHash(bytes::const_span authKey) {
	Expects(authKey.size() == kKeySize);

	const auto sha1 = openssl::Sha1(authKey);
	const auto sha256 = openssl::Sha256(authKey);
	auto result = bytes::vector(36);
	bytes::copy(result, bytes::make_span(sha1).subspan(0, 16));
	bytes::copy(
		bytes::make_span(result).subspan(16),
