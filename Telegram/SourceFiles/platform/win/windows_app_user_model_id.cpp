...
#ifdef OS_WIN_STORE
const WCHAR AppUserModelIdBase[] = L"Mercurygram.MercurygramDesktop.Store";
#else // OS_WIN_STORE
const WCHAR AppUserModelIdBase[] = L"Mercurygram.MercurygramDesktop";
#endif // OS_WIN_STORE
...
void CleanupShortcut() {
	...
	QString path = systemShortcutPath() + u"Mercurygram.lnk"_q;
...
bool checkInstalled(QString path = {}) {
	...
	const auto installed = u"Mercurygram Desktop/Mercurygram.lnk"_q;
	const auto old = u"Mercurygram Win (Unofficial)/Mercurygram.lnk"_q;
...
bool ValidateShortcut() {
	...
	if (cAlphaVersion()) {
		path += u"MercurygramAlpha.lnk"_q;
