...
#include "data/data_types.h" // MessageFlags
...
// Media::ttlSeconds for a self-destruct media bubble that is shown covered
// (view-once style: a photo or non-round video with a ttl of at most a
// minute), 0 when it renders plainly. Same rule for both directions.
[[nodiscard]] int32 SecretChatMediaCoverTtl(
	int32 ttl,
	PhotoData *photo,
	DocumentData *document);

// Local MsgId of a reply target, but only when it actually belongs to this
// chat (the compose bar can hold a reply aimed at another peer); 0 otherwise.
// Shared by the text / file / existing-document secret-chat send paths.
[[nodiscard]] MsgId LocalReplyToMsgId(FullMsgId replyMessageId, PeerId chatId);
...
public:
	...
	// Send a plain text message into a ready secret chat. When replyToMsgId is a
	// local message in this chat, transmit it as reply_to_random_id (and show
	// the quote on the local echo).
	// `silent` (here and below) is the composer's "send without sound": set
	// on the decryptedMessage and on the outer sendEncrypted* call.
	void sendText(
	...
	// afterSetRefetch precedes replyToMsgId so a stray positional bool can't bind
	// to MsgId (MsgId's operator bool is explicit, so the reverse can't misbind).
	...
	// An existing server photo (an inline result picked from the GIFs tab) has
	// no server-reference form in the secret layer -- only stickers have one --
	// so its bytes are re-uploaded encrypted, like GIFs in
	// uploadExistingDocument. Waits for the large size to be in memory, then
	// hands it to the normal prepare-and-send-file pipeline, which comes back
	// here through Api::SendConfirmedFile -> sendFile.
	...
	// Location, venue and contact have their own secret-layer media, so they
	// travel as a plain (non-upload) encrypted message. The secret layer has no
	// vcard and no venue_type, and its contact user_id is int32 where the
	// modern one is int64; all three match what the Android client sends.
	void sendLocation(
		not_null<SecretChatData*> chat,
		float64 lat,
		float64 lon,
	...
	void messagesRead(int32 chatId, TimeId maxDate, TimeId date);
	...
	// Also re-renders the chat's screenshot notices (same name dependency).
	void refreshTtlNotices(not_null<SecretChatData*> chat);

	// The user opened / consumed self-destruct media (photo or video viewed,
	// voice or round video played): arm their timers now and tell the partner
	// via decryptedMessageActionReadMessages so their copies start counting
	// too. Called from ApiWrap::markContentsRead, which reads a whole batch at
	// a time -- one service action for the batch, not one per message (each
	// costs an out_seq_no and a round trip).
	void contentRead(
		not_null<SecretChatData*> chat,
		const std::vector<not_null<HistoryItem*>> &items);

	// The local history was cleared (ApiWrap::clearHistory / deleteHistory):
	// drop the per-message indices so nothing stale is persisted or resolved.
	void historyCleared(not_null<SecretChatData*> chat);

	// In-flight handshake material kept until the key is computed. Persisted
	// (Storage::Account) so a restart between requestEncryption and the peer's
	// accept, or between the peer's request and our accept, can still finish.
	struct Pending {
	...
	// persisted so a restart mid-exchange does not brick the chat.
	struct Rekey {
	...
	[[nodiscard]] const Pending *pending(int32 chatId) const;
	void restorePending(int32 chatId, Pending pending);
	[[nodiscard]] const Rekey *rekey(int32 chatId) const;
	void restoreRekey(int32 chatId, Rekey rekey);
	...
	// the peer de-duplicates the resend against the original. Persisted with
	// the chats (keyed by out_seq_no) so a resend request that arrives after a
	// restart is still answered with the message instead of a tombstone.
	struct SentLayer {
		bytes::vector serialized;
		uint64 randomId = 0;
		bool isService = false;
		// The encrypted file the original message carried, as the server
		// returned it (0 = a message without one). A resend re-attaches it as
		// inputEncryptedFile, since the layer itself does not carry it.
		uint64 fileId = 0;
		uint64 fileAccessHash = 0;
	};
	[[nodiscard]] const std::map<int32, SentLayer> *sentLayers(
		int32 chatId) const;
	void restoreSentLayers(int32 chatId, std::map<int32, SentLayer> layers);

	// Encrypted updates that arrived while the chat had no key yet, with the
	// qts each carried (see flushPendingMessages). The qts checkpoint moves
	// past them as soon as they are parked, so they are persisted with the
	// chats and restored on the next launch: the server never resends an
	// update it has seen ACKed.
	using PendingMessage = std::pair<MTPEncryptedMessage, int32>;
	[[nodiscard]] const std::vector<PendingMessage> *pendingMessages(
		int32 chatId) const;
	void restorePendingMessages(
		int32 chatId,
		std::vector<PendingMessage> messages);

	// After the chats are restored from disk: re-announce to every ready chat
	// whose last announce predates the current layer (client upgrade), like
	// MessagesController.java does when loading its encrypted chats.
	void notifyLayerIfOutdated();
	// After both blobs are restored: replay the parked updates of a chat that
	// already has its key. The accept path persists the chat as Ready (the
	// notify-layer send writes it) before it flushes them, so a quit in
	// between leaves an ACKed list the server never resends.
	void flushRestoredPendingMessages();
...
private:
	...
	struct IncomingFile {
		...
		// Local MsgId of the replied-to message (resolved from
		// reply_to_random_id), 0 if none/unknown; set on the created item.
		MsgId replyToMsgId = 0;
		bool silent = false; // decryptedMessage.silent: notify without sound
		// decryptedMessage.grouped_id: album membership (0 = standalone).
		// Set on the created item so Desktop lays consecutive members out as
		// one album, the way Android groups them.
		uint64 groupedId = 0;
		...
		int32 ttl = 0;
		// Self-destruct media whose timer starts when the user OPENS it (not
		// when the chat is read): a photo / non-round video with a ttl of at
		// most a minute is shown covered (view-once style), and any voice / round
		// video with a ttl arms on playback. Mirrors the mobile clients, which
		// exclude media from the read-triggered timer start.
		bool coverOnOpen = false;
		...
		// Redone from a persisted record (restoreMessages): the placeholder
		// had already been viewed, and its countdown was already running
		// (absolute destroy time, 0 = not armed). Both zero for a live one.
		bool read = false;
		TimeId ttlDestroyAt = 0;
		QByteArray data; // accumulated ciphertext
		int64 offset = 0;
		// The message this file belongs to was deleted (by either side), the
		// history was cleared or the chat was closed while the parts were still
		// coming: stop fetching, write nothing and build no bubble.
		bool cancelled = false;
		// The decrypted message and the server file it arrived with: the
		// download is persisted from these while it runs (serializeMessages)
		// and redone from them on the next launch, since its qts was ACKed
		// on receipt and the server never delivers the message again.
		decrypted::MTPDecryptedMessage message;
		MTPEncryptedFile file;
		int attempts = 0; // failed part requests, for the bounded retry
	};
	...
	struct OutgoingFile {
		...
		TextWithEntities caption;
		// Local MsgId of the replied-to message (0 if none); resolved to
		// reply_to_random_id on the wire and to .replyTo on the local echo.
		MsgId replyToMsgId = 0;
		...
		bool asVoice = false;
		int32 duration = 0;
		QByteArray waveform; // voice note, 5-bit wire encoding (may be empty)
		...
		int64 cipherSize = 0;
		bool silent = false; // "send without sound" on the wire message
		// Album membership (SendingAlbum::groupId, 0 = standalone): sent as
		// decryptedMessage.grouped_id on every member. Android groups the
		// consecutive messages sharing it into one album; there is no
		// multi-media call in the secret layer, each member travels alone.
		uint64 groupedId = 0;
		// The self-destruct timer this message was prepared with: the chat ttl
		// floored at the media duration (EffectiveMediaTtl), captured once so
		// the local bubble and the wire message agree even when the chat timer
		// changes while the upload is still running.
		int32 ttl = 0;
	};
	...
	void ensureDhConfig(Fn<void()> done, Fn<void()> fail = nullptr);
	...
	// Outgoing bubbles carry MessageFlag::BeingSent (the clock) until the
	// server confirms the messages.sendEncrypted*; a rejected send shows the
	// red failed mark instead (and drops its on-disk copy). `date` is the
	// server's timestamp from the confirmation, stamped on the bubble.
	void markSecretSent(FullMsgId itemId, TimeId date);
	void markSecretSendFailed(FullMsgId itemId);

	// Shared wire boilerplate of the plain (non-upload) send paths: build the
	// decryptedMessage carrying `text` and `wireMedia`, serialize + remember +
	// encrypt it, send it, then add the local echo with `localMedia` and
	// account for the key use. decryptedMessageMediaEmpty as `wireMedia` means
	// a text-only message (f_media is set from it, never a default-constructed
	// boxed media: its type() asserts). Holds the send back through
	// queuePendingSend() while the chat is not established yet.
	void sendDecryptedMessage(
		not_null<SecretChatData*> chat,
		const TextWithEntities &text,
	...
	// "X took a screenshot!" (partner's action, incoming only): a centered
	// service message, persisted as StoredKind::ServiceScreenshot. Not a
	// notification source (no ClientSideUnread), like the TTL notice.
	void addScreenshotNotice(not_null<SecretChatData*> chat, TimeId date);
	not_null<HistoryItem*> buildScreenshotServiceMessage(
		not_null<SecretChatData*> chat,
		TimeId date,
		MsgId id);
	[[nodiscard]] TextWithEntities screenshotNoticeText(
		not_null<SecretChatData*> chat) const;
	...
	// so each message destroys at readTime + ttl.
	...
	// out-of-order messages the accepted one unblocks. `countKeyUse` charges
	// the message against the current key's PFS counter once it passes the
	// duplicate check (a replayed message must not inflate it).
	...
	// Messages that arrived while the chat had no key yet (our
	// acceptEncryption / the peer's accept still in flight) are held and
	// replayed once the key is established, mirroring Android's
	// pendingSecretMessages.
	void flushPendingMessages(not_null<SecretChatData*> chat);

	// An animated / video sticker referenced by set from the peer is rendered
	// only after the set is fetched and the document id is found in it, like
	// Android's verifyAnimatedStickerMessage: until then the item shows as a
	// plain file (the peer's bytes never reach the Lottie / video decoder).
	void verifyExternalSticker(
		FullMsgId itemId,
	...
	// A download redone from its persisted record passes the state its
	// placeholder had reached (see IncomingFile::read / ttlDestroyAt).
	void processDecryptedMessage(
		not_null<SecretChatData*> chat,
		const decrypted::MTPDecryptedMessage &message,
		TimeId date,
		const MTPEncryptedFile *encryptedFile,
		bool restoredRead = false,
		TimeId restoredDestroyAt = 0);
	...
	// Drop the cached content of messages that must not be retransmitted any
	// more (deleted, cleared, self-destructed): handleResend then answers
	// their seq with a delete tombstone, so the peer's sequence stays intact
	// while the text and file keys leave the persisted blob. Service entries
	// are kept (a resent rekey / delete action carries no user content).
	void retireSentLayers(int32 chatId, Fn<bool(const SentLayer&)> match);
	...
	// Non-destructive forward lookup (MsgId -> random_id), used when composing a
	// reply so we can fill reply_to_random_id for the replied-to message.
	[[nodiscard]] std::optional<uint64> randomIdByMsg(
		PeerId chatId,
		MsgId msgId) const;
	// Resolve a local-echo reply target and stamp its message flags: marks
	// self-destruct media unsavable (MessageFlag::NoForwards when ttlSeconds > 0;
	// pass 0 for copyable text) and adds MessageFlag::HasReplyInfo when the
	// replied-to message is still present locally. Returns its FullReplyTo (empty
	// when absent).
	[[nodiscard]] FullReplyTo resolveLocalReply(
		PeerId chatId,
		MsgId replyToMsgId,
		int32 ttlSeconds,
	...
	// Common message fields for a bubble built from an incoming file, shared by
	// the up-front thumb placeholder and the finished-download item.
	[[nodiscard]] HistoryItemCommonFields incomingFields(
		not_null<SecretChatData*> chat,
	...
	// Mark the chat's in-flight downloads matching `match` (null = all of them)
	// as cancelled, so the parts still on the wire are dropped instead of
	// resurrecting a message that is already gone.
	void cancelIncomingFiles(
		int32 chatId,
		Fn<bool(const IncomingFile&)> match = nullptr);
	// Forget a finished (or cancelled) download: it leaves the persisted
	// message blob with the next write.
	void dropIncomingFile(not_null<IncomingFile*> state);

	// Delete the on-disk encrypted copy and the cache mirror of a secret-chat
	// media message (self-destructed, deleted, history cleared or send failed).
	void removeSecretFile(not_null<const HistoryItem*> item);

	// At-rest encryption for decrypted secret-chat media: stored under the
	// account's secret_files/ dir encrypted with the local key (Storage::File),
	// never as plaintext. Read back to memory on demand (see SecretFileLoader /
	// restore).
	...
	void writeLocal();
	// Queue writeLocal() for the end of the current event loop turn, for a
	// burst of per-item changes (a cleared history retires every sent layer).
	...
	// Queue writeMessagesLocal() for the end of the current event loop turn.
	// Every message event (send, receive, read, remove) re-serializes every
	// secret chat and fsyncs, so a burst -- a batch content-read, a cleared
	// history -- must cost one write, not one per message.
	void scheduleMessagesWrite();
	...
	// Encrypted updates received before the key was ready (keyed by
	// secretChatId), replayed by flushPendingMessages once it is.
	base::flat_map<int32, std::vector<PendingMessage>> _pendingMessages;
	...
	// Resend ranges already asked from the peer (keyed by secretChatId, then
	// start -> end out_seq_no), so every buffered message behind the same gap
	// does not burn another out_seq_no on a duplicate request (Android
	// requestedHoles). Entries die once the peer fills them.
	base::flat_map<int32, base::flat_map<int32, int32>> _requestedHoles;
	...
	// In-flight incoming file downloads (keyed by secretChatId), owned here
	// until they finish or are cancelled so serializeMessages can persist
	// them; a download that gave up stays, and the next launch redoes it.
	base::flat_map<
		int32,
		std::vector<std::shared_ptr<IncomingFile>>> _incomingFiles;
	...
	// Screenshot notices (service messages), persisted as
	// StoredKind::ServiceScreenshot.
	base::flat_map<PeerId, base::flat_set<MsgId>> _screenshotNotices;
	...
	// A message change is waiting for the queued write; writeLocal() flushes
	// it first so the chats blob never gets ahead of the messages blob.
	bool _messagesDirty = false;
	SingleQueuedInvokation _messagesWrite;
	SingleQueuedInvokation _localWrite;
