...
, _idleFinishTimer([=] { checkIdleFinish(); }) {
	...
	api().request(MTPupdates_GetState(
		...
		const auto currentQts = result.c_updates_state().vqts().v;
		stateDone(result);
		// The secret chats (and their qts checkpoint) may still be loading
		// from disk; the catch-up runs once both sides are here.
		_secretCatchUpQts = currentQts;
		if (_secretChatsRestored) {
			secretCatchUp(currentQts);
		}
	}).send();
...
void Updates::secretChatsRestored() {
	_secretChatsRestored = true;
	if (_secretCatchUpQts) {
		secretCatchUp(*_secretCatchUpQts);
	}
}

void Updates::secretCatchUp(int32 currentQts) {
...
void Updates::differenceDone(const MTPupdates_Difference &result) {
	...
	case mtpc_updates_differenceTooLong: {
		// The server dropped the gap: adopt its pts, reload the dialogs and
		// pull whatever is left (the qts stream for secret chats included)
		// from the new state, as the mobile clients do.
		const auto &d = result.c_updates_differenceTooLong();
		const auto pts = d.vpts().v;
		const auto changed = (pts != _ptsWaiter.current());
		_ptsWaiter.init(pts);
		_ptsWaiter.setRequesting(false);
		session().api().requestDialogs();
		if (changed) {
			MTP_LOG(0, ("getDifference { good - after differenceTooLong }%1"
				).arg(_session->mtp().isTestMode() ? " TESTMODE" : ""));
			getDifference();
		} else {
			// getDifference() cancelled the no-updates watchdog; without a
			// follow-up request nothing re-arms it, and the session would
			// never re-sync on its own if the socket went quiet.
			_lastUpdateTime = crl::now();
			_noUpdatesTimer.callOnce(kNoUpdatesTimeout);
		}
	} break;
...
void Updates::feedUpdate(const MTPUpdate &update) {
	...
	case mtpc_updateNewEncryptedMessage: {
		...
		// Keep the local qts current: an unrelated getDifference would
		// otherwise start from a stale qts and replay this session's whole
		// secret stream.
		_updatesQts = std::max(_updatesQts, qts);
	} break;
