...
// peer sending "Application/X-TgSticker" would otherwise slip past.
...
// plain file. Returns the set the sticker attribute referenced (empty when
// there was none).
[[nodiscard]] MTPInputStickerSet StripStickerAttributes(
		QVector<MTPDocumentAttribute> &attributes) {
	auto set = MTPInputStickerSet(MTP_inputStickerSetEmpty());
...
, _mtp(&api->instance())
, _messagesWrite([=] { writeMessagesLocal(); }) {
}
...
// waveform after a Document record, and adds the screenshot notice kind.
constexpr auto kSecretMessagesVersion = qint32(3);
...
void EncryptedChats::restoreMessages(const QByteArray &serialized) {
	...
	auto migrated = false;
	// On every exit path (a truncated blob returns from inside the loops).
	const auto rewrite = gsl::finally([&] {
		if (migrated) {
...
void EncryptedChats::sendText(
		...
		MsgId replyToMsgId,
		bool silent) {
	// A text carrying a link travels with decryptedMessageMediaWebPage so
	// the peer renders a preview from the first URL (Android does the same
	// from its own preview, or from the first URL entity for inline text).
	// The local echo stays plain: this client never resolves previews for
	// secret chats (that would name the link to the server in plaintext).
	auto wireMedia = decrypted::MTPDecryptedMessageMedia(
		decrypted::MTP_decryptedMessageMediaEmpty());
	for (const auto &entity : text.entities) {
		if (entity.type() == EntityType::Url) {
			wireMedia = decrypted::MTP_decryptedMessageMediaWebPage(
				MTP_string(text.text.mid(entity.offset(), entity.length())));
			break;
		}
	}
...
void EncryptedChats::newMessage(
		...
		int32 qts) {
	// Set when the update was only parked in _pendingMessages (RAM only): the
	// qts checkpoint must NOT advance past it, or ACKing it to the server and
	// persisting the new checkpoint would lose the message for good if we quit
	...
	// A parked message leaves the checkpoint where it is, so the server keeps
	// re-delivering it until we really have the key.
	if (!buffered) {
		setQts(qts);
	}
}
...
void EncryptedChats::processDecryptedMessage(
	...
	message.match([&](
		...
		if (const auto entities = fields.ventities()) {
			text.entities = EntitiesFromDecrypted(entities->v);
		}
		...
		if (const auto media = fields.vmedia()) {
			...
			const auto startMedia = [&](
				...
				// A playable media must outlive its own duration (Android
				// floors the ttl at duration + 1).
				auto ttl = fields.vttl().v;
				if (ttl > 0 && duration > 0) {
					ttl = std::max(duration + 1, ttl);
				}
			...
			media->match([&](
				...
				if (IsAnimatedStickerMime(mime)) {
					...
					// against a server set, so they render as a file, never
					// as a sticker. Both the attributes AND the mime have to
					// go: the decoders are chosen from the mime alone.
					[[maybe_unused]] const auto set = StripStickerAttributes(
						attributes);
				...
				if (!trusted && IsAnimatedStickerMime(mime)) {
					...
					if (set.type() != mtpc_inputStickerSetEmpty) {
						verify = set;
...
void EncryptedChats::startSelfDestructTimers(
	...
	for (auto j = i->second.begin(); j != i->second.end();) {
		...
		// Media that arms on open / playback (covered photo or video, voice,
		// round video) waits for contentRead() (ours) or the partner's
		// readMessages action (handleReadMessages), not the chat read: the
		// mobile clients exclude it from the max_date read too, else our
		// copy would burn before the partner ever opened it.
		if (SelfDestructMode(ttl, item).unread) {
			++j;
			continue;
		}
...
void EncryptedChats::flushPendingMessages(not_null<SecretChatData*> chat) {
	...
	auto held = std::move(i->second);
	_pendingMessages.erase(i);
