...
// random_id) right after the common header.
constexpr auto kSecretMessagesVersion = qint32(2);
...
void EncryptedChats::sendText(
		...
		const QString &text,
		MsgId replyToMsgId) {
	if (queuePendingSend(chat, [=] { sendText(chat, text, replyToMsgId); })) {
		return;
	}
...
void EncryptedChats::sendExistingDocument(
	...
	if (queuePendingSend(chat, [=] {
		sendExistingDocument(chat, document, caption, afterSetRefetch, replyToMsgId);
	})) {
		return;
	}
	...
	if (!sticker) {
		...
		uploadExistingDocument(chat, document, caption, replyToMsgId);
		return;
	}
	if (!shortName.isEmpty()) {
		...
		sendExternalDocument(chat, document, caption, shortName, replyToMsgId);
		return;
	}
	if (!setId || !setAccessHash) {
		...
		if (!afterSetRefetch) {
			refetchRecentAndFavedSets([=] {
				sendExistingDocument(chat, document, caption, true, replyToMsgId);
			});
			return;
		}
		sendStickerAsDocument(chat, document, caption, replyToMsgId);
		return;
	}
	_mtp.request(MTPmessages_GetStickerSet(
		...
		if (name.isEmpty()) {
			...
			sendExternalDocument(chat, document, caption, name, replyToMsgId);
		}
	}).fail([=](const MTP::Error &error) {
...
void EncryptedChats::sendExternalDocument(
	...
	ensureInDialogs(chat);
	const auto history = _session->data().history(chat->id);
	auto itemFlags = MessageFlags(MessageFlag::Local | MessageFlag::Outgoing);
	const auto replyTo = resolveLocalReply(
		chat->id,
		replyToMsgId,
		chat->ttl(),
...
FullMsgId EncryptedChats::addLocalDocumentBubble(
	...
	ensureInDialogs(chat);
	const auto history = _session->data().history(chat->id);
	auto flags = MessageFlags(MessageFlag::Local | MessageFlag::Outgoing);
	const auto replyTo = resolveLocalReply(
		chat->id,
		replyToMsgId,
		chat->ttl(),
