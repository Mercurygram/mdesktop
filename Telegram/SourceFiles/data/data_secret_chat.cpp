...
void SecretChatData::setState(SecretChatState state) {
	...
	// The chat list row shows the state as its subtitle (stateText()).
...
QString SecretChatData::stateText() const {
	switch (_state) {
	case SecretChatState::Requested:
		return tr::lng_secret_chat_waiting(
			tr::now,
			lt_user,
			_user ? _user->shortName() : QString());
	case SecretChatState::Empty:
	case SecretChatState::Waiting:
		return tr::lng_secret_chat_exchanging(tr::now);
	case SecretChatState::Discarded:
		return tr::lng_secret_chat_cancelled(tr::now);
	case SecretChatState::Ready:
		return QString();
	}
	Unexpected("State in SecretChatData::stateText.");
}

QString SecretChatData::startedText() const {
	return _amCreator
		? tr::lng_secret_chat_started_outgoing(
			tr::now,
			lt_user,
			_user ? _user->shortName() : QString())
		: tr::lng_secret_chat_started_incoming(tr::now);
}
...
void SecretChatData::setKey(bytes::const_span key, uint64 fingerprint) {
	...
	_hasKey = true;
	if (_keyHash.empty()) {
		_keyHash = MTP::SecretChat::ComputeKeyHash(key);
	}
...
void SecretChatData::setKeyHash(bytes::const_span hash) {
	_keyHash = bytes::make_vector(hash);
}

void SecretChatData::clearKey() {
	bytes::set_with_const(_key, gsl::byte{});
	_keyFingerprint = 0;
	_hasKey = false;
	_keyHash.clear();
	_keyCreationDate = 0;
...
void SecretChatData::setPreviousKey(
		bytes::const_span key,
		uint64 fingerprint) {
	Expects(key.size() == kKeySize);

	bytes::copy(_previousKey, key);
	_previousKeyFingerprint = fingerprint;
	_hasPreviousKey = true;
}

void SecretChatData::clearPreviousKey() {
	bytes::set_with_const(_previousKey, gsl::byte{});
	_previousKeyFingerprint = 0;
	_hasPreviousKey = false;
}

bytes::const_span SecretChatData::previousKey() const {
	Expects(_hasPreviousKey);
