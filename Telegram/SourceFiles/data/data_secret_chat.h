...
public:
	...
	// Wipe the key material (a discarded chat must not decrypt anything that
	// arrives late for its id).
	void clearKey();
	...
	// The 36-byte verification hash (MTP::SecretChat::ComputeKeyHash) of the
	// key the handshake established. setKey() fills it only the first time,
	// so a PFS rekey leaves it alone and the identicon keeps matching the
	// partner's, which the mobile clients also compute once. Persisted; a
	// chat stored by a build without it gets the hash of its current key.
	void setKeyHash(bytes::const_span hash);
	[[nodiscard]] bytes::const_span keyHash() const {
		return _keyHash;
	}

	// The key retired by the last PFS rekey commit. The peer may still have
	// messages in flight under it (it switches only once our commitKey lands),
	// so newMessage() falls back to it; the first message decrypted with the
	// current key drops it (mirrors Android's future_auth_key after commit).
	void setPreviousKey(bytes::const_span key, uint64 fingerprint);
	void clearPreviousKey();
	[[nodiscard]] bool hasPreviousKey() const {
		return _hasPreviousKey;
	}
	[[nodiscard]] bytes::const_span previousKey() const;
	[[nodiscard]] uint64 previousKeyFingerprint() const {
		return _previousKeyFingerprint;
	}
	...
	// The layer the peer last announced (8 = never announced anything).
	[[nodiscard]] int32 layer() const {
		return _layer;
	}
	// The layer we last announced to the peer (0 = never), so a client
	// upgrade re-announces once instead of on every start.
	void setAnnouncedLayer(int32 layer) {
		_announcedLayer = layer;
	}
	[[nodiscard]] int32 announcedLayer() const {
		return _announcedLayer;
	}

	// Composer notice / chat list subtitle for a chat that is not Ready yet
	// (or was cancelled), phrased like the mobile clients. Empty when Ready.
	[[nodiscard]] QString stateText() const;
	// Chat list subtitle for a Ready chat with no messages yet.
	[[nodiscard]] QString startedText() const;

	// Also mirrored into PeerData::messagesTTL() so the upstream auto-delete
	// UI (composer timer button, chat list userpic badge) shows it.
	...
	// peer's out_seq_no). Persisted with the chat blob next to the sent-layer
	// cache so the resend clamp in handleResend survives a restart; a blob
	// without it reads back 0, which just disables the clamp. Mirrors
...
private:
	...
	bytes::vector _keyHash;
	uint64 _previousKeyFingerprint = 0;
	MTP::AuthKey::Data _previousKey = { { gsl::byte{} } };
	TimeId _keyCreationDate = 0;
	int32 _keyUseCountOut = 0;
	int32 _keyUseCountIn = 0;
	int32 _layer = 8;
	int32 _announcedLayer = 0;
	int32 _ttl = 0;
	int32 _sentCount = 0;
	int32 _inSeqNo = 0;
