...
void DocumentData::setSecretEncryptedLocation(const QString &path) {
	...
	// A view created before the file was attached (live receive shows the
	// bubble first) already asked for an automatic load and got nothing;
	// ask again now that the encrypted path exists so it decrypts and plays.
	if (const auto media = activeMediaView()) {
		media->automaticLoad(Data::FileOrigin(), nullptr);
	}
}

bool DocumentData::isSecretEncrypted() const {
	return !_secretEncryptedPath.isEmpty();
}
