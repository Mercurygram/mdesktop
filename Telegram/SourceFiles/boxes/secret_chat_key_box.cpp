...
[[nodiscard]] std::array<QString, 4> FormatKeyHashLines(
	...
	for (auto row = 0; row != 4; ++row) {
		...
		for (auto col = 0; col != 8; ++col) {
			...
			line += QString::number(
				std::to_integer<uchar>(data[row * 8 + col]),
...
// Draws the secret-chat key identicon, matching the official mobile clients:
// the 36-byte key hash (SecretChatData::keyHash) split into 2-bit groups
// (LSB first) filled left-to-right, top-to-bottom into a 12x12 grid using
// four fixed colors. See core.telegram.org and Android IdenticonDrawable.
[[nodiscard]] QImage GenerateKeyImage(bytes::const_span data) {
	...
	for (auto y = 0; y != kGridSide; ++y) {
		for (auto x = 0; x != kGridSide; ++x) {
			const auto value = (std::to_integer<uchar>(data[bitOffset / 8])
				>> (bitOffset % 8)) & 0x3;
...
void SecretChatKeyBox(
	...
	auto image = GenerateKeyImage(chat->keyHash());
	...
	const auto lines = FormatKeyHashLines(chat->keyHash());
