...
bool History::shouldBeInChatList() const {
	if (peer->migrateTo() || !folderKnown()) {
		...
		// message resolves to a known-null. A chat the partner cancelled keeps
		// its row while it still has history ("Secret chat cancelled"); one
		// closed with the history deleted leaves the list
		// (updateChatListExistence re-checks this).
