...
#   2. adds a "Version X.Y.Z.N[ beta]" commit (set_release_version.py);
#   3. tags that commit vX.Y.Z.N[-beta].
#
# N is the fork revision: 1 for the first mirror of an upstream base, and 2, 3,
# ... for later cuts off the same base (BUMP_REVISION, see below).
#
...
#                     open conflict issue) and force-replacing its tag. dev is
#                     only moved when FORCE_TAG is itself the newest upstream
#                     release, so re-cutting an old base never drags dev back.
...
# Highest fork revision already mirrored for an upstream tag on the given
# channel suffix ("" or "-beta"); empty when none. Anchored on the suffix so the
# stable and beta series are counted separately, and so the "^{}" deref lines of
# annotated tags are ignored.
max_revision() { # <upstream-tag> <suffix>
	git ls-remote --tags origin "refs/tags/$1.*" \
		| sed -n "s#.*refs/tags/$1\.\([0-9]\+\)$2\$#\1#p" \
		| sort -n | tail -1
}
...
mirror_one() {
	...
	# Pick the fork revision. A forced re-mirror without BUMP_REVISION replaces
	# the newest existing revision (re-cut a build that failed CI); with it, the
	# next revision is cut instead, so the version string changes and the
	# autoupdater actually offers the build to clients already on the old one.
	local latest rev
	latest="$(max_revision "$up" "$suffix")"; latest="${latest:-0}"
	...
	local our_tag="${up}.${rev}${suffix}"
	local issue_title="mirror conflict: ${our_tag}"

	# Idempotency is "any revision of this base exists", not "vX.Y.Z.1 exists":
	# after a .2 the scheduled run would otherwise find no .1 and re-cut.
	if [ -z "$FORCE_TAG" ] && [ "$latest" -gt 0 ]; then
		log "already mirrored: ${up}${suffix} (latest .${latest}, skip)"
		return 0
	fi
	...
	if [ "$expect_beta" = 1 ]; then
		msg="Beta version ${up#v}.${rev}."
	else
		msg="Version ${up#v}.${rev}."
	fi
...
canary_run() {
	...
	log "canary: replaying $(git rev-list --count "${MG_BASE}..${MG_SRC}") MG commits onto upstream/dev (${tip})"
	git checkout -q --detach "$MG_SRC"
