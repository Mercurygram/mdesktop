...
#  - For C++ sources/headers a merge driver (configure_merge_driver()) resolves
#    the content merge with git's own three-way `git merge-file` (xdiff) instead
#    of the rebase's default `ort` strategy. This is required, not cosmetic: ort's
#    internal merge is *zealous* -- for an MG hunk that lands next to an upstream
#    change (the common case once upstream relocates the code an MG patch hooks
#    into) ort can silently resolve the whole file to the upstream side, dropping
#    the MG insertion with no conflict markers. `git merge-file` instead keeps
...
# Register a merge driver that resolves C++ sources/headers with git's own
...
configure_merge_driver() {
	...
	log "registered git merge-file (xdiff) merge driver for C++ sources"
}
