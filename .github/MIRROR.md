...
| --------------------- | --------------------- | ------------- |
| stable `vX.Y.Z`       | tag `vX.Y.Z.N`        | `release.yml` |
| pre-release `vX.Y.Z`  | tag `vX.Y.Z.N-beta`   | `release.yml` |
...
`N` is the fork revision. The first mirror of an upstream base is `.1`; later
cuts off the same base are `.2`, `.3`, ... See *Manual / forced re-mirror*.
...
<upstream vX.Y.Z>
  ...
  + Version X.Y.Z.N[ beta]        <- set_release_version.py, tag points here
```
...
- `MG_SRC` is `origin/dev` with the version commit(s) the previous mirror left
  on its tip stripped off. `dev` carries its release's `Version X.Y.Z.N` commit
...
Rebase `dev` onto the latest upstream **release tag** (`vX.Y.Z`), the target
this mirror replays onto, not the `upstream/dev` tip; that keeps the set lean
and lets conflicts be resolved once on `dev`. Detection stays exact at any base
(merge-base locates the fork point regardless).
...
checks (and any open `mirror-conflict` issue for the tag). Two modes:

- **`bump_revision` off** (default): force-replaces that release's existing
  tag. Use it to re-cut a build that failed CI. The version string does not
  change, so clients already on it see no update.
- **`bump_revision` on**: cuts the next fork revision instead (`vX.Y.Z.1` exists
  -> pushes `vX.Y.Z.2`). Use it to ship new `dev` commits on top of an upstream
  base that is already released; the higher `AppVersion` makes the autoupdater
  offer the build.

Either way the cut is re-derived from scratch: the mirror replays the MG commits
onto the upstream tag, so a bump picks up whatever is on `dev` at that moment.
Nothing cherry-picked onto an old tag by hand survives.

`dev` moves only when `force_tag` is itself the newest upstream release, so
re-cutting an older base -- a fix release on the last stable while `dev` already
tracks a newer pre-release -- never drags `dev` backwards.
