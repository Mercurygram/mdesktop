...
- **`bump_revision` off** (default): force-replaces that release's existing
  branch and tag. Use it to re-cut a build that failed CI. The version string
  does not change, so clients already on it see no update.
...
Either way the `release/X.Y.Z` branch is re-derived from scratch: the mirror
checks out `origin/dev` and replays the MG commits onto the upstream tag, so a
bump picks up whatever is on `dev` at that moment. Nothing cherry-picked onto
the release branch by hand survives.
